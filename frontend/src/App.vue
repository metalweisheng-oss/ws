...
const changelog = [
  {
    ...
    items: [
      '新增「籌碼」分頁（位於權證後）：輸入股票代號，顯示三大法人（外資/投信/自營）近 1/3/5/10 日買賣超（張）、融資餘額及日增減、集保大戶持股比例，並附近10日逐日明細表。資料來源：TWSE T86（三大法人）、MI_MARGN（融資融券）、集保 TDCC；T86 依日期全市場快取，首次查詢需約 3–5 秒，後續查詢秒回。',
    ]
  },
  {
  ...
  },
  {
    ...
    items: [
      '漲跌排行：觀察名單個股新增「時序」按鈕，點擊後顯示該股當日各快照時段的成交量、漲停委買量、委買比與五日量比，方便追蹤委買量隨時間的變化',
      ...
      '權證查詢：「符合條件」篩選預設調整為剩餘天數 > 60 天、成交量 > 30 張',
      '漲跌排行：「觀察時段」下拉預設自動選取當日最新時段，切換日期後同樣自動跳到該日最新',
  ...
  },
  {
    date: '2026-05-19', tag: '修正',
    items: [
      '權證：情境分析表格漲跌幅排序改為由上到下 +10% → -10%',
      '漲跌排行：修正 13:30 快照資料不完整導致觀察名單顯示異常的問題——MIS 於收盤後進入結算模式，資料殘缺；後端已排除 13:30 快照（最後一筆改為 13:00），前端若偵測到最新快照股票數不足前一筆的 50% 時自動退回前一筆',
      '漲跌排行：修正漲停委買量在即時排行中顯示空白的問題——原本 DB 每 30 分鐘快照儲存的漲停委買量未被帶入即時資料，現在當 MIS 即時委買量無資料時自動 fallback 到當日快照累積最大值',
